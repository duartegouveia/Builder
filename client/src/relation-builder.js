...
// Apply filters for a specific instance
function applyFiltersForInstance(st) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const [colIdxStr, filter] of Object.entries(getFilters(st))) {
      ...
      if (filter.type === 'indices') {
        const indicesSet = filter.indices instanceof Set ? filter.indices : new Set(filter.indices);
        if (!indicesSet.has(i)) {
          passes = false;
          break;
        }
      } else if (filter.type === 'values') {
        const valuesArr = Array.isArray(filter.values) ? filter.values : [...filter.values];
        const matches = valuesArr.some(v => {
          if (v === null) return value === null;
          if (value === null) return false;
          return String(v) === String(value);
        });
        ...
        if (!matchesCriteriaForInstance(value, filter.criteria, colIdx, st)) {
          passes = false;
          break;
        }
      }
...
function matchesCriteriaForInstance(value, criteria, colIdx, st) {
  ...
  // String criteria
  if (type === 'string' && criteria.contains) {
    if (!String(value).toLowerCase().includes(criteria.contains.toLowerCase())) return false;
  }
...
function switchViewForInstance(st, view) {
  ...
  if (view === 'table') {
    ...
    renderPivotForInstance(st);
  } else if (view === 'correlation') {
    renderCorrelationForInstance(st);
  } else if (view === 'diagram') {
    renderDiagramForInstance(st);
  } else if (view === 'ai') {
    renderAIForInstance(st);
  } else if (view === 'saved') {
    renderSavedForInstance(st);
  }
}
...
// Render Pivot view for a specific instance
function renderPivotForInstance(st) {
  const container = st.container;
  const pivotWrapper = container.querySelector('.pivot-view-wrapper');
  if (!pivotWrapper || !st.relation) return;
  
  // Get columns that can be used for pivot
  const cols = st.columnNames.map((name, idx) => ({ idx, name, type: st.columnTypes[idx] }));
  const numericCols = cols.filter(c => ['int', 'float'].includes(c.type));
  
  let html = `
    <div class="pivot-config">
      <div class="pivot-config-row">
        <label>Rows:</label>
        <select class="pivot-rows pivot-select">
          <option value="">None</option>
          ${cols.map(c => `<option value="${c.idx}">${escapeHtml(c.name)}</option>`).join('')}
        </select>
      </div>
      <div class="pivot-config-row">
        <label>Columns:</label>
        <select class="pivot-cols pivot-select">
          <option value="">None</option>
          ${cols.map(c => `<option value="${c.idx}">${escapeHtml(c.name)}</option>`).join('')}
        </select>
      </div>
      <div class="pivot-config-row">
        <label>Values:</label>
        <select class="pivot-value-col pivot-select">
          <option value="">Count</option>
          ${numericCols.map(c => `<option value="${c.idx}">${escapeHtml(c.name)}</option>`).join('')}
        </select>
        <select class="pivot-value-agg pivot-select">
  ...
  pivotWrapper.innerHTML = html;
  
  // Event listener for generate button
  pivotWrapper.querySelector('.btn-generate-pivot')?.addEventListener('click', () => {
    generatePivotForInstance(st);
  });
}

function generatePivotForInstance(st) {
  const container = st.container;
  const pivotWrapper = container.querySelector('.pivot-view-wrapper');
  
  const rowColIdx = pivotWrapper.querySelector('.pivot-rows')?.value;
  const colColIdx = pivotWrapper.querySelector('.pivot-cols')?.value;
  const valueColIdx = pivotWrapper.querySelector('.pivot-value-col')?.value;
  const aggType = pivotWrapper.querySelector('.pivot-value-agg')?.value || 'count';
  
  const hasRows = rowColIdx !== '' && rowColIdx !== null;
  const hasCols = colColIdx !== '' && colColIdx !== null;
  ...
  const rowIdx = hasRows ? parseInt(rowColIdx) : null;
  const colIdx = hasCols ? parseInt(colColIdx) : null;
  const valIdx = valueColIdx !== '' ? parseInt(valueColIdx) : null;
  ...
  rowValuesArr.forEach(rv => {
    pivotData[rv] = {};
    colValuesArr.forEach(cv => {
      pivotData[rv][cv] = aggType === 'count' ? 0 : [];
    });
  });
  ...
  getSortedIndices(st).forEach(i => {
    ...
    if ((hasRows ? rv !== null : true) && (hasCols ? cv !== null : true)) {
      if (aggType === 'count') {
        pivotData[rv][cv]++;
      } else if (valIdx !== null) {
        const val = row[valIdx];
        if (typeof val === 'number') {
          pivotData[rv][cv].push(val);
        }
      }
    }
  });
  ...
  rowValuesArr.forEach(rv => {
    results[rv] = {};
    colValuesArr.forEach(cv => {
      const data = pivotData[rv][cv];
      if (aggType === 'count') {
        results[rv][cv] = data;
      } else if (aggType === 'sum') {
        results[rv][cv] = Array.isArray(data) ? data.reduce((a, b) => a + b, 0) : 0;
      } else if (aggType === 'average') {
        results[rv][cv] = Array.isArray(data) && data.length > 0 ? (data.reduce((a, b) => a + b, 0) / data.length).toFixed(2) : 0;
      }
    });
  });
  ...
  colValuesArr.forEach(cv => {
    tableHtml += `<th>${escapeHtml(cv)}</th>`;
  });
  tableHtml += '</tr></thead><tbody>';
  
  rowValuesArr.forEach(rv => {
    tableHtml += `<tr><th>${escapeHtml(rv)}</th>`;
    colValuesArr.forEach(cv => {
      tableHtml += `<td>${results[rv][cv]}</td>`;
    });
    tableHtml += '</tr>';
  });
  ...
  const tableContainer = pivotWrapper.querySelector('.pivot-table-container');
  if (tableContainer) tableContainer.innerHTML = tableHtml;
}

// Render Correlation view for a specific instance
function renderCorrelationForInstance(st) {
  const container = st.container;
  const corrWrapper = container.querySelector('.correlation-view-wrapper');
  if (!corrWrapper || !st.relation) return;
  
  const cols = st.columnNames.map((name, idx) => ({ idx, name, type: st.columnTypes[idx] }));
  const numericCols = cols.filter(c => ['int', 'float', 'boolean'].includes(c.type));
    ...
    <div class="correlation-config">
      <div class="correlation-section">
        ...
        <div class="correlation-config-row">
          ...
          <select class="corr-col-x pivot-select">
        ...
        <div class="correlation-config-row">
          ...
          <select class="corr-col-y pivot-select">
            <option value="">Select...</option>
            ${numericCols.map(c => `<option value="${c.idx}">${escapeHtml(c.name)}</option>`).join('')}
          </select>
        </div>
  ...
  corrWrapper.querySelector('.btn-calculate-corr')?.addEventListener('click', () => {
    calculateCorrelationForInstance(st);
  });
}
...
function calculateCorrelationForInstance(st) {
  ...
  const xIdx = parseInt(corrWrapper.querySelector('.corr-col-x')?.value);
  const yIdx = parseInt(corrWrapper.querySelector('.corr-col-y')?.value);
  const method = corrWrapper.querySelector('.corr-method')?.value || 'pearson';
  ...
  getSortedIndices(st).forEach(i => {
    const row = st.relation.items[i];
    const x = typeof row[xIdx] === 'number' ? row[xIdx] : (row[xIdx] === true ? 1 : row[xIdx] === false ? 0 : null);
    const y = typeof row[yIdx] === 'number' ? row[yIdx] : (row[yIdx] === true ? 1 : row[yIdx] === false ? 0 : null);
    if (x !== null && y !== null) {
      pairs.push({ x, y });
    }
  });
  ...
  if (pairs.length < 2) {
    corrWrapper.querySelector('.correlation-result').innerHTML = '<p class="text-muted">Not enough data pairs</p>';
    return;
  }
  ...
  if (method === 'spearman') {
    // Spearman: rank-based
    const rankX = getRanks(pairs.map(p => p.x));
    const rankY = getRanks(pairs.map(p => p.y));
    correlation = pearsonFromPairs(rankX.map((rx, i) => ({ x: rx, y: rankY[i] })));
  } else {
    // Pearson
    correlation = pearsonFromPairs(pairs);
  }
  
  const strength = Math.abs(correlation) < 0.3 ? 'Weak' : Math.abs(correlation) < 0.7 ? 'Moderate' : 'Strong';
  const direction = correlation > 0 ? 'positive' : 'negative';
  
  corrWrapper.querySelector('.correlation-result').innerHTML = `
    <div class="correlation-result-card">
      <div class="correlation-value">${correlation.toFixed(4)}</div>
      <div class="correlation-interpretation">${strength} ${direction} correlation</div>
      <div class="correlation-details">n = ${pairs.length} pairs</div>
    </div>
  `;
}
...
// Render Diagram view for a specific instance
function renderDiagramForInstance(st) {
  const container = st.container;
  const diagramWrapper = container.querySelector('.diagram-view-wrapper');
  if (!diagramWrapper || !st.relation) return;
  
  let html = `
    <div class="diagram-info">
      <p class="diagram-description">This diagram uses <strong>t-SNE</strong> to visualize data similarity. Similar rows cluster together, while dissimilar rows are placed further apart. Each circle represents a row, colored by cluster assignment.</p>
    </div>
  ...
  diagramWrapper.querySelector('.btn-run-clustering')?.addEventListener('click', () => {
    runClusteringForInstance(st);
  });
}

function runClusteringForInstance(st) {
  const container = st.container;
  const diagramWrapper = container.querySelector('.diagram-view-wrapper');
  ...
  if (n > maxPoints) {
    if (!confirm('You have ' + n + ' rows. t-SNE with more than ' + maxPoints + ' points may be slow. Continue?')) {
      return;
    }
  }
  
  const numClusters = parseInt(diagramWrapper.querySelector('.tsne-clusters')?.value) || 5;
  const perplexity = parseInt(diagramWrapper.querySelector('.tsne-perplexity')?.value) || 30;
  const iterations = parseInt(diagramWrapper.querySelector('.tsne-iterations')?.value) || 500;
  const progressEl = diagramWrapper.querySelector('.tsne-progress');
  const canvas = diagramWrapper.querySelector('.diagram-canvas');
  
  if (!canvas) return;
  
  // Get columns for similarity calculation (same logic as main)
  const cols = [];
  ...
  setTimeout(() => {
    ...
    st.diagramNodes = nodes;
    st.diagramBallRadius = ballRadius;
    
    if (progressEl) progressEl.style.display = 'none';
    
    // Render diagram
    renderDiagramForInstanceCanvas(st, canvas);
  }, 50);
}

function renderDiagramForInstanceCanvas(st, canvas) {
  const ctx = canvas.getContext('2d');
  const nodes = st.diagramNodes || [];
  ...
  ctx.fillStyle = '#666';
  ctx.font = '12px sans-serif';
  ctx.fillText('Rows: ' + nodes.length + ' | Clusters: ' + clusters.length, 10, canvas.height - 40);
}

// Render AI view for a specific instance
function renderAIForInstance(st) {
  const container = st.container;
  const aiWrapper = container.querySelector('.ai-view-wrapper');
  if (!aiWrapper || !st.relation) return;
  ...
  const askBtn = aiWrapper.querySelector('.btn-ai-ask');
  const input = aiWrapper.querySelector('.ai-question');
  
  askBtn?.addEventListener('click', () => {
    const question = input?.value.trim();
    if (question) {
      askAIForInstance(st, question);
    }
  });
...
async function askAIForInstance(st, question) {
  const container = st.container;
  const aiWrapper = container.querySelector('.ai-view-wrapper');
  ...
  try {
    ...
    if (result.type === 'filter' && result.conditions) {
      responseDiv.innerHTML = `
        ...
        <div class="ai-filter-result">
          <span>Filter: ${result.conditions.map(c => `${c.column} ${c.operator} ${c.value ?? ''}`).join(' AND ')}</span>
          <button class="btn-apply-ai-filter btn btn-sm btn-outline">Apply Filter</button>
        </div>
      `;
        ...
        result.conditions.forEach(cond => {
          ...
          if (colIdx >= 0) {
            if (cond.operator === 'equals') {
              getFilters(st)[colIdx] = { type: 'values', values: [cond.value] };
            } else if (cond.operator === 'contains') {
              const matching = [];
              st.relation.items.forEach((row, i) => {
                if (String(row[colIdx]).toLowerCase().includes(String(cond.value).toLowerCase())) {
                  matching.push(row[colIdx]);
                }
              });
              getFilters(st)[colIdx] = { type: 'values', values: [...new Set(matching)] };
            }
          }
        });
        applyFiltersForInstance(st);
        switchViewForInstance(st, 'table');
      });
    } else if (result.answer) {
      responseDiv.innerHTML = `<div class="ai-answer-text">${escapeHtml(result.answer)}</div>`;
    } else {
      responseDiv.innerHTML = '<div class="ai-answer-text">No specific answer generated.</div>';
    }
  } catch (err) {
    responseDiv.classList.remove('loading');
    responseDiv.innerHTML = `<p class="ai-error">Error: ${escapeHtml(err.message)}</p>`;
  }
}

// Render Saved view for a specific instance
function renderSavedForInstance(st) {
  const container = st.container;
  const savedWrapper = container.querySelector('.saved-view-wrapper');
