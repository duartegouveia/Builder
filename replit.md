...
- **Relation Builder**: An advanced data table interface for creating and managing relational data with diverse views (Table, Cards, Pivot, Analysis, AI).
...
- **rel_options Synchronization**: When adding new `rel_options` properties, they must be explicitly copied in BOTH `parseRelation()` and `initRelationInstance()` functions using nullish coalescing (`??`) with `DEFAULT_REL_OPTIONS` fallback. Current properties include: `editable`, `show_multicheck`, `show_natural_order`, `show_id`, `show_column_kind`, `show_stats`, `show_hierarchy`, `hierarchy_column`, `hierarchy_root_value`, `hierarchy_initial_value`, `single_item_mode`, `label_field_top_down`, `OnDoubleClickAction`, `general_view_options`, `general_always_visible_options`, `general_line_options`, `general_multi_options`, `show_filter_comparison`, `show_filter_criteria`, `show_filter_topn`, `show_filter_outliers`, `show_binning`, `show_derived_columns`, `show_cartesian_product`, `show_formatting`, `show_groupby`.
- **Table Features**: Includes pagination, row selection, multi-column sorting with Sort Panel (drag-drop reorder, per-column options: Case/Accent/Punctuation Insensitive, Parse Numbers, using `Intl.Collator` with `'und'` locale), advanced filtering (by values, nulls, top/middle/bottom N, percentage-based filtering, outliers), conditional formatting (color scale with legend, data bars, icons, color current results), binning for numeric columns, and per-column statistics (count, min, max, mean, std dev, quartiles, violin plots with KDE overlay).
- **Grouping and Nesting**: Supports grouping by columns (including Group By ALL columns with keep-visible mode), infinite nesting of relations, and complex data hierarchies. Group By is disabled for id columns.
...
- **View Tabs**: Offers multiple views:
    ...
    - **Pivot Table View**: Cross-tabulation with aggregation options (count, sum, average, median, std dev, % total/row/col). Includes integrated **Pivot Chart** panel below the table: Canvas-based charting with 5 chart types (bar vertical, bar horizontal, line, area, pie), grouped/stacked bar modes, and Excel-like chart options (title, axis titles, axis formatting with label rotation, data labels, error bars ±stddev, gridlines, legend with position, trendlines). Trendline fitting supports linear, polynomial 2nd/3rd degree, exponential, and logarithmic regression with auto-selection by best R² fit. Smart defaults auto-generate chart title from pivot dimensions, axis titles from column names, and select best-fit trendline. Toggle buttons to independently show/hide table and chart panels. Chart download in PNG (via canvas.toDataURL) and GIF (custom LZW encoder with 256-color quantization) formats.
    - **Analysis View**: Unified statistical analysis with 4 sub-tabs:
      - **Pairwise**: 24 correlation/association methods (Pearson, Spearman, Kendall, Point-Biserial, Phi, Cramér's V, Eta², Mutual Information, Distance Correlation, Polyserial, Polychoric, Tetrachoric, Biserial, Rank-Biserial, Somers' D, Gamma, Lambda, Tau-c, Uncertainty Coefficient, Hoeffding's D, Blomqvist's Beta, Chatterjee's Xi, Cosine Similarity, Jaccard Index) with auto-detect, scatter plots, and strength interpretation. Analyze All Pairs computes all combinations.
      - **Matrix**: Correlation Matrix heatmap (Pearson/Spearman/Kendall) and Mutual Information Matrix with color-coded cells.
      - **Clustering**: t-SNE dimensionality reduction with k-means clustering visualization. Configurable clusters, perplexity, iterations. "Clusters as Column" exports cluster, cluster_x, cluster_y columns.
      - **Multivariate**: PCA (eigenvalues, scree plot, loading matrix, 2D scatter), Factor Analysis (principal axis factoring, communalities), Canonical Correlation (dual column group selection), MANOVA (Wilks' Lambda, Pillai's Trace).
      - Results stored in `uiState.analysisResult` for AI view integration.
...
- **Derived Columns**: Extraction system for creating new columns: date extractions (Year, Month, Day, Weekday, Quarter, Semester, Day/Week of Year, ISO Week), time extractions (Hour, Minute, Second, AM/PM, Hour12), float rounding, string metrics (Length, Bytes, Flesch Reading Ease, Flesch-Kincaid Grade, Sentences), hierarchy column derivations (Ascendants, Descendants, Path).
- **Hierarchy Navigation**: Breadcrumb trail showing path from root to current level with counts (#direct >> #total descendants), (ⓘ) info badge, "Show all descendants" toggle, `hierarchy_initial_value` in rel_options for custom start level.
